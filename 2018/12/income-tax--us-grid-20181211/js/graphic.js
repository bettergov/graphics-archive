...
var renderStateGridMap = function(config) {
	...
	_.each(config['data'], function(state) {
		if (state[valueColumn] !== null) {
			...
			var activeClass =
				state[valueColumn] != 0 ? 'state-active' : 'state-inactivate';

			chartElement
				.select('.' + stateClass)
				.attr('class', stateClass + ' ' + activeClass + ' ' + categoryClass)
	...
	chartElement
		...
		.text(function(d) {
			...
			var stateLabel = isMobile ? state['usps'] : state['ap'];

			return d['income_tax_type'] === 'flat' ? stateLabel + '*' : stateLabel;
		...
		.attr('y', function(d) {
			...
			return isMobile
				? tileBox['y'] + tileBox['height'] * 0.5 + textOffset
				: tileBox['y'] + tileBox['height'] * 0.35 + textOffset;
		});
	if (!isMobile) {
		chartElement
			.append('g')
			...
			.attr('y', function(d) {
				...
				return tileBox['y'] + tileBox['height'] * 0.7 + textOffset;
			});
	}
};
