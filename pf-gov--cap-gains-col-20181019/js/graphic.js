...
var renderGroupedStackedColumnChart = function(config) {
  ...
  var colorScale = d3
    .scaleOrdinal()
    .domain(
      d3.keys(config['data'][0]).filter(function(d) {
  ...
  xScale.domain().forEach(function(c, k) {
    ...
    var columns = categoryElement
      .selectAll('.columns')
      .data(
        config['data'].filter(function(d, i) {
          return d['category'] == c;
        })
      )
      .enter()
    ...
    // axis labels
    var xAxisBars = d3.axisBottom(xScaleBars).tickFormat(function(d) {
      return d;
    });
    columns
      .append('g')
