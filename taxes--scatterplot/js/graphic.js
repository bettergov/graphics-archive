 ...
 */
var xColumn = 'General sales tax per capita',
    yColumn = 'Income tax per capita';

var xFormat = function (d) { return '$' + d.formatMoney(0);},
    yFormat = function (d) { return '$' + d.formatMoney(0);};
...
var renderDotChart = function (config) {
    ...
    var fixedTooltip = containerElement.append('div')
        .style('position', 'absolute')
        .style('z-index', '9')
        .attr('class', 'tooltip')
        .text('Illinois');
     ...
     */
    var xDomain = d3.extent(config.data, function (d) { return d.x;});

    var yDomain = d3.extent(config.data, function (d) { return d.y;});
    ...
    var bestFitData = function (data) {
        ...
        var m = (N * sumColumn('xy') - sumColumn('x') * sumColumn('y')) / (N * sumColumn('x2') - sumColumn('x') * sumColumn('x'));
    ...
    chartElement.append('g')
      ...
      .enter().append("circle")
        ...
        .on("mousemove", function (d) {
            return tooltip.style('top', (yScale(d.y) - 3) + "px")
    ...
    if (includeTrend) {
        ...
        var bestFitData = bestFitData(config.data);
        
        trendElement.append("path")
            .datum(bestFitData)
            .attr("class", "line least-squares")
            .attr("fill", "none")
            .attr("stroke", COLORS['blue2'])
        ...
        trendElement.append("text")
            .attr("x", xScale(bestFitData[1][0]))
            .attr("y", yScale(bestFitData[1][1]))
            .attr("dy", ".6em")
     ...
     */
    var point = _.findWhere(config.data, {'state_name': 'Illinois'});
    fixedTooltip.text('Illinois')
